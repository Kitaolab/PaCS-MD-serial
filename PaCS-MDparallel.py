...
#gmxcmd="mpirun -n 1 gmx_mpi "  #gmx serial calling
#gmxcmd2="mpirun -n "+str(runmode)+" gmx_mpi "#call MPI task in this variable
...
if rest<0:
	...
	if gpu>=0 and ntomp>0:
		os.system(gmxcmd+" mdrun -deffnm "+outfn+"-0-0/"+outfnpf+"-0-0 -v -ntomp "+str(ntomp)+" -gpu_id "+str(gpuid))
	else:
		os.system(gmxcmd+" mdrun -deffnm "+outfn+"-0-0/"+outfnpf+"-0-0 -v -ntomp "+str(ntomp))
	...
	if runmode==1:
		...
		for x in range(0,mdloop):
			...
			for m in range(x*runmode+1,(x+1)*runmode+1):
				multidir=multidir+outfn+"-"+str(n)+"-"+str(m)+"/"+outfnpf+"-"+str(n)+"-"+str(m)+" "
			if gpu>=0 and ntomp>0:
				os.system(gmxcmd2+" mdrun -deffnm -multidir "+multidir+" -v -ntomp "+str(ntomp)+" -gpu_id "+str(gpuid))
			else:
				os.system(gmxcmd2+" mdrun -deffnm -multidir "+multidir+" -v -ntomp "+str(ntomp))		
